...
describe('Modelos de IA por categoria (/api/admin/ai-models)', () => {
  ...
  it('as opções são exatamente as 3 do avaliador e as 4 do paciente', async () => {
    const admin = await loginAs('admin');
    const res = await request(app).get('/api/admin/ai-models').set(authHeader(admin));
    expect(res.body.avaliadorOpcoes.map((o) => o.key)).toEqual(['gpt-5.5', 'gpt-5.4', 'glm-5.2']);
    expect(res.body.pacienteOpcoes.map((o) => o.key))
      .toEqual(['gpt-5.4-mini', 'glm-5.2', 'gpt-5.4', 'claude-sonnet-5']);
    // Todo avaliador em 'high'; paciente só o GLM raciocina.
  ...
  it('escolher um modelo persiste, marca fonte admin e reflete no GET', async () => {
    const admin = await loginAs('admin');
    const put = await request(app).put('/api/admin/ai-models').set(authHeader(admin))
      .send({ categoria: 'treinamento', evaluator: 'gpt-5.4', patient: 'claude-sonnet-5' });
    expect(put.status).toBe(200);
    expect(cat(put.body, 'treinamento').avaliador.preset).toBe('gpt-5.4');
    ...
    expect(cat(get.body, 'treinamento').avaliador.model).toBe('gpt-5.4-2026-03-05');
  ...
  describe('compatibilidade com a escolha antiga do visitante', () => {
    ...
    it('GET /api/settings continua respondendo o que roda de fato pro visitante', async () => {
      const admin = await loginAs('admin');
      await request(app).put('/api/admin/ai-models').set(authHeader(admin))
        .send({ categoria: 'visitante', evaluator: 'gpt-5.4' });

      const aluno = await loginAs('aluno');
      const res = await request(app).get('/api/settings').set(authHeader(aluno));
      expect(res.body.visitorEvaluationModel).toBe('gpt-5.4');
    });
  });
});
...
describe('ai-models (resolve puro)', () => {
  ...
  it('casa o preset pelo prefixo do modelo, sobrevivendo à troca de pin', () => {
    const spec = aiModels.resolveEvaluator('neuro', {}, {
      model: 'gpt-5.4-9999-99-99', provider: 'openai', effort: 'low',
    });
    expect(spec.preset).toBe('gpt-5.4');
    expect(spec.model).toBe('gpt-5.4-9999-99-99'); // o modelo real não é reescrito
